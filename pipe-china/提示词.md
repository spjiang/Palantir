...
但检测到 [ 下面 有花括号{,就代表对象数据下面抽到实体数据，
就开始 new_obj_data_str 就开始从{保存字符串，
如：new_obj_data_str = "{"type": "has_sensor", "src": "管段", "dst": "传感器", "props": {}}"
当每个花括号开始成对后，就代表此对象数据接收完毕了，这就是 json 数据结构特性啊，都是以左右花括号代表一个对象。利用这个机制进行数据提取。提取当前这一个对象后，就开始生成 html 内容前端进行展示了。
