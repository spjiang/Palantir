...
- `demo-os/compose.yaml`：Docker Compose 编排文件
...
- **Docker Compose 插件**：支持 `docker compose` 命令
...
| 服务 | 容器名 | 端口 | 说明 |
|---|---|---:|---|
| 前端 | `frontend` | 8080 | 访问入口 |
| 后端 API | `api` | 8000 | Swagger：`/docs` |
| 智能体服务 | `agent` | 8001 | Swagger：`/docs` |
| 小模型服务 | `model-service` | 8002 | Swagger：`/docs` |
| PostgreSQL | `postgres` | 5432 | 演示业务库 |
| Redis | `redis` | 6379 | 缓存/队列 |

> 如端口冲突，请修改 `compose.yaml` 对应 `ports:` 映射。
...
- 智能体：`http://localhost:8001/docs`
- 小模型：`http://localhost:8002/docs`
...
- PostgreSQL 使用数据卷 `pgdata` 持久化（见 `compose.yaml`）。
...
现象：启动失败提示端口被占用。  
处理：修改 `compose.yaml` 的端口映射，或停止占用端口的程序。
