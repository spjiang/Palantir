# 应急安全AI操作系统方案书 V8（城市暴雨内涝指挥｜全量版）

本文件 = **V7 完整正文** + **V8 工程化增强与平台治理补齐**。

- **V7 部分**：保持原结构与内容（作为方案主干）
- **V8 增强部分**：从工程交付/可运维/可规模化角度补齐（对标 Foundry/Gotham/Apollo）
...
- **1) 风险热力图滚动更新（未来 1–3 小时）**
  ...
  - **谁来完成（组件责任）**：
    - **数据平台/接入服务**：采集雨量/雷达/水位/泵站/路况，写入 Raw/ODS/TSDB（Raw=原始落地层；ODS=清洗对齐后的操作型明细层；TSDB=时序数据库/指标时序存储），打 DQ 标签
    - **模型服务**：生成网格/路段风险（含 `confidence`、`explain_factors[]`），提供热力图/TopN 接口，confidence：置信度/可信度，表示模型对该风险判断有多“确定”（通常 0～1 或 0～100，值越大越可信）。
explain_factors：解释因子/可解释特征列表，用来说明“为什么判成这个风险”（例如：雨强上升、水位逼近警戒、泵站故障、低洼地形、历史易涝点、路况拥堵影响排涝 等）。
...
- **3) 一键生成“布防/封控/联动”任务包并下发**
  ...
  - **谁来完成（组件责任）**：
    - **智能体服务**：把“研判建议 + 依据引用”结构化为可派单的任务包（`TaskPack`/`tasks[]`），并调用工作流派单接口落库下发；其中每条 `task` 至少包含：`owner_org`（责任单位/处置队伍）、`sla_minutes`（完成时限/SLA）、`required_evidence[]`（回执必传证据，如定位/照片/视频/测量值）、`need_approval`（是否需要人审/审批后才能下发），可选 `title/detail` 用于任务标题与处置说明。
...
> 说明：部分 Markdown 预览器不支持 Mermaid 渲染。这里提供**纯文本分层图**（任何环境都可显示），结构为“自上而下分层 + 主链路”。

```text
┌────────────────────────────── L0 界面层（谁在用） ──────────────────────────────┐
│  指挥端前端/UI（热力图、对象联动、对话研判、派单监控、战报导出）                │
│  移动端/处置人员（接单、回执、定位/照片/视频/表单）                            │
└───────────────────────────────────────────────────────────────────────────────┘
                │（调用/查看）                         │（回执）
                ▼                                      ▼
┌──────────────────────── L1 数据接入与治理（1.4 数据设计） ────────────────────────┐
│  接入连接器/适配：雨量/雷达/水位/泵站/路况/事件                                  │
│  Raw/ODS/TSDB 落库：DQ 标签、窗口聚合、统一口径                                 │
└───────────────────────────────────────────────────────────────────────────────┘
                │（供模型/本体/追溯使用）
                ▼
┌────────────────────────── L2 语义与状态（1.3 本体 Ontology） ────────────────────┐
│  本体：实体/关系/责任片区/空间归属                                               │
│  对象状态快照：get_object_state（地图叠加、对象联动、责任归属）                   │
└───────────────────────────────────────────────────────────────────────────────┘
                │
                ▼
┌────────────────────────── L3 风险推理（模型服务 1.9.3） ─────────────────────────┐
│  风险推理：risk_score / risk_level                                                │
│  解释&置信度：confidence / explain_factors                                        │
│  服务接口：热力图 / TopN                                                          │
└───────────────────────────────────────────────────────────────────────────────┘
                │（TopN + 解释 + 置信度）
                ▼
┌────────────────────────── L4 智能体决策（1.6/1.9.4） ────────────────────────────┐
│  RAG 证据检索：预案/规程/历史战报（“有依据的建议”）                               │
│  任务包编排：create_task_pack -> TaskPack/tasks[]                                 │
│  简报草稿（可选）：draft_briefing                                                  │
└───────────────────────────────────────────────────────────────────────────────┘
                │（任务包下发）
                ▼
┌────────────────────────── L5 执行闭环（工作流 1.7/1.9.5） ────────────────────────┐
│  风控门禁/人审审批：封控/停运/跨部门联动等高风险动作需审核                        │
│  派单&状态机：SLA/升级/通知/回执校验(required_evidence)                           │
│  证据库/对象存储：附件引用 ID（照片/视频/定位等）                                  │
└───────────────────────────────────────────────────────────────────────────────┘
                │（时间线与指标）
                ▼
┌────────────────────────── L6 战报与追溯（1.9.6/1.4.6） ──────────────────────────┐
│  时间线汇总：预警/模型/智能体/人审/派单/回执                                      │
│  指标计算&战报导出：任务完成率、响应时长等                                         │
│  血缘&审计引用：lineage / version_refs（支撑“依据可追溯”）                        │
└───────────────────────────────────────────────────────────────────────────────┘

主链路（从研判到闭环）：
接入/落库 → 风险推理(解释&置信度) → 智能体任务包 → 审批/派单状态机 → 移动端回执(证据) → 战报(时间线/指标/追溯)
```
